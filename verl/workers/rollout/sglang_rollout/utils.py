...
def broadcast_pyobj(
    ...
    if rank == src:
        if len(data) == 0:
            ...
            tensor_data = torch.ByteTensor(np.frombuffer(serialized_data, dtype=np.uint8)).to(device)  # gpu broadcast
            # tensor_data = torch.ByteTensor(np.frombuffer(serialized_data, dtype=np.uint8))  # cpu broadcast
